...
import { SongBlock } from '@/entities/song';
import { TonalitySelect } from '@/features/select-tonality';
    ...
    <div className={cs(styles.song, className)}>
      <div className={styles.song}>
        <div className={styles.flowBtns}>
          ...
          <button onClick={handleHideChords} className={styles.flowBtns__hide}>
        ...
        {song.blocks.map((block, i) => (
          <div key={i} className={styles.block}>
            <span className={styles.block__title}>{block.title}</span>
            {!!block.text.length && (
              <SongBlock
                block={block}
